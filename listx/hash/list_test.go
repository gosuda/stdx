...
// createHashList is a factory function for creating HashList instances
func createHashList[T any]() listx.List[T] {
	return hash.New[T]()
}

func TestHashList_Add(t *testing.T) {
	testListAdd(t, createHashList[int])
}

func TestHashList_Insert(t *testing.T) {
	testListInsert(t, createHashList[int])
}

func TestHashList_Get(t *testing.T) {
	testListGet(t, createHashList[int])
}

func TestHashList_Set(t *testing.T) {
	testListSet(t, createHashList[int])
}

func TestHashList_Remove(t *testing.T) {
	testListRemove(t, createHashList[int])
}

func TestHashList_RemoveElement(t *testing.T) {
	testListRemoveElement(t, createHashList[int])
}

func TestHashList_IndexOf(t *testing.T) {
	testListIndexOf(t, createHashList[int])
}

func TestHashList_LastIndexOf(t *testing.T) {
	testListLastIndexOf(t, createHashList[int])
}

func TestHashList_Contains(t *testing.T) {
	testListContains(t, createHashList[int])
}

func TestHashList_Size(t *testing.T) {
	testListSize(t, createHashList[int])
}

func TestHashList_IsEmpty(t *testing.T) {
	testListIsEmpty(t, createHashList[int])
}

func TestHashList_Clear(t *testing.T) {
	testListClear(t, createHashList[int])
}

func TestHashList_ToSlice(t *testing.T) {
	testListToSlice(t, createHashList[int])
}

func TestHashList_ForEach(t *testing.T) {
	testListForEach(t, createHashList[int])
}
...
func testListGet(t *testing.T, factory func() listx.List[int]) {
	...
	if valOpt.IsNone() || valOpt.Unwrap() != 20 {
		t.Errorf("Expected Get(1) to return Some(20), got %v", valOpt)
	}
...
func testListRemove(t *testing.T, factory func() listx.List[int]) {
	...
	if result.IsErr() || result.Unwrap() != 20 {
		t.Errorf("Expected Remove(1) to return Ok(20), got %v", result)
	}
	...
	if valOpt.IsNone() || valOpt.Unwrap() != 10 {
		t.Errorf("Expected first element to be Some(10), got %v", valOpt)
	}

	valOpt = l.Get(1)
	if valOpt.IsNone() || valOpt.Unwrap() != 30 {
		t.Errorf("Expected second element to be Some(30), got %v", valOpt)
	}

	// Test out of bounds
	result = l.Remove(-1)
	if result.IsOk() {
		t.Error("Remove with negative index should return Err")
	}

	result = l.Remove(2)
	if result.IsOk() {
		t.Error("Remove with too large index should return Err")
	}
}
...
func testListIndexOf(t *testing.T, factory func() listx.List[int]) {
	...
	if indexOpt.IsNone() || indexOpt.Unwrap() != 0 {
		t.Errorf("Expected IndexOf(10) to return Some(0), got %v", indexOpt)
	}

	indexOpt = l.IndexOf(20)
	if indexOpt.IsNone() || indexOpt.Unwrap() != 1 {
		t.Errorf("Expected IndexOf(20) to return Some(1), got %v", indexOpt)
	}

	indexOpt = l.IndexOf(99)
	if indexOpt.IsSome() {
		t.Errorf("Expected IndexOf(99) to return None, got %v", indexOpt)
	}
}
...
func testListLastIndexOf(t *testing.T, factory func() listx.List[int]) {
	...
	if indexOpt.IsNone() || indexOpt.Unwrap() != 2 {
		t.Errorf("Expected LastIndexOf(10) to return Some(2), got %v", indexOpt)
	}

	indexOpt = l.LastIndexOf(20)
	if indexOpt.IsNone() || indexOpt.Unwrap() != 1 {
		t.Errorf("Expected LastIndexOf(20) to return Some(1), got %v", indexOpt)
	}

	indexOpt = l.LastIndexOf(99)
	if indexOpt.IsSome() {
		t.Errorf("Expected LastIndexOf(99) to return None, got %v", indexOpt)
	}
}
